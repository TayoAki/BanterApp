...
| M12 | Learn through the complete curriculum | implemented_unverified (published for pilot) | 30 lesson seeds and 30 prompts load; 10 Notice recognition keys authored as drafts (`packages/content/src/recognition.ts`). On 2026-09-20 the owner asked for prompts and lessons to be available to testers, so `publication/production.json` now lists all 10 frameworks, 30 lessons, 30 prompts and 20 examples as `published` (each entry can be set to `retired` to pull it without a code change). **Still open**: the formal curriculum review and the source-rights decision for the verbatim example quotes; both are recorded here as owner items, not resolved by publishing. |
...
5. Curriculum review of seeds and Notice keys and the source-rights decision for quoting the framework PDFs. The production manifest already publishes everything for the pilot; retire entries there if the review says so.
